function accuracies = run(data, k, train_params, kernel, fileID)
...
details = '%s\tTest-Chunk:%d\tNumberOfSupportVector:%d\n'; % for the text file
...
for i=1:k
    ...
    for j=1:k
        ...
        c_max = cost(ci_max(1));
        imagesc(gamma, cost, val_accuracies);
        fn = sprintf('%s-Validation-Chunk%d.jpg', kernel, i);
        title(fn);
        ylabel('Cost (c) parameter')
        ...
        fwrite(fileID, sprintf(details, kernel, i, model.totalSV));
...
fn = sprintf('%s-Accuracy-Figure.jpg', kernel);
print('-djpeg', fn);
